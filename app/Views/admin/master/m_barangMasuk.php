                ...
                <div class="white_card_header">
                    <div class="box_header m-0">
                        <div class="main-title">
                            <h3 class="m-0">Transaksi Barang Masuk</h3>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="white_box_tittle list_header">
                            ...
                            <div class="box_right d-flex lms_block">
                                <div class="add_button ms-2">
                                    <a href="/MasterBarangMasuk/create" class="btn_1">Tambah Data</a>
                                </div>
                            </div>
                        </div>
                        <div class="QA_table mb_30">

                            <table class="table table-responsive" border="solid" id="table-barangMasuk">
                                ...
                                <tbody>
                                    <?php $no = 1;
                                    foreach ($tampildata as $mbm) : ?>
                                        <tr>
                                            <td><?= $no++ ?></td>
                                            <td><?= $mbm['nama_brg'] ?></td>
                                            <td><?= $mbm['nama_mitra'] ?></td>
                                            <td><?= $mbm['jumlah_masuk'] ?></td>
                                            <td><?= $mbm['unit'] ?></td>
                                            <td><?= $mbm['tanggal_masuk'] ?></td>
                                            <td><?= $mbm['jenis_tr'] ?></td>
                                            <td>
                                                <a href="#" data-id="<?= $mbm['kd_tr'] ?>" id="BTN_STATUS_UPDATE" class="btn <?= $mbm['status'] == 'Belum Selesai' ? 'status_btn_red' : 'status_btn' ?>">
                                                    <?= $mbm['status'] ?>
                                                </a>
                                            </td>
                                        </tr>
...
<script>
    $(document).on("click", "#BTN_STATUS_UPDATE", async function(e) {
        ...
        try {
            const response = await $.post("<?= base_url('/Api/BarangMasuk/status') ?>", {
                id
            });
